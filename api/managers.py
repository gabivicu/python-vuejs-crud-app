...
class ItemQuerySet(models.QuerySet):
    ...
    def overdue(self):
        ...
        return self.filter(due_date__lt=timezone.now(), completed=False)

    def due_today(self):
        """Return items due today."""
        today = timezone.now().date()
        return self.filter(due_date__date=today, completed=False)

    def due_upcoming(self, days=7):
        """Return items due in the next N days."""
        now = timezone.now()
        future = now + timedelta(days=days)
        return self.filter(due_date__gte=now, due_date__lte=future, completed=False)
    ...
    def with_stats(self):
        ...
        return self.annotate(
            days_until_due=models.Case(
                models.When(
                    due_date__isnull=False, then=F("due_date") - timezone.now()
                ),
                default=None,
    ...
    def high_priority_pending(self):
        ...
        return self.filter(priority__in=["high", "urgent"], completed=False)
